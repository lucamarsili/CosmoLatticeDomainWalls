#ifndef TEMPLAT_LATTICE_MEASUREMENTS_SCALING_H
#define TEMPLAT_LATTICE_MEASUREMENTS_SCALING_H
...
namespace TempLat {
  ...
  public:
      ...
      Scaler(const T& pT,SpaceStateInterface::SpaceType spaceType):
      mT(pT),
  ...
  auto average(T instance, SpaceStateInterface::SpaceType spaceType = GetGetReturnType<T>::isComplex ?  SpaceStateInterface::SpaceType::Fourier : SpaceStateInterface::SpaceType::Configuration) {
    ...
    scale(T instance, SpaceStateInterface::SpaceType spaceType = GetGetReturnType<T>::isComplex ?  SpaceStateInterface::SpaceType::Fourier : SpaceStateInterface::SpaceType::Configuration) {
        return Scaler<T>(instance, spaceType).compute_s();
    }

    auto scale(ZeroType a)
    {
        return 0;
    }

    template <typename T>
    auto getScale(T instance, SpaceStateInterface::SpaceType spaceType = GetGetReturnType<T>::isComplex ?  SpaceStateInterface::SpaceType::Fourier : SpaceStateInterface::SpaceType::Configuration) {
        return Scaler<T>(instance, spaceType);
    }
