...
private:
    ...
    double mu ,lambda; //here the only parameters are the self interacting coupling lambda and the vev.
  ...
  public:
    ...
    Model<MODELNAME>(parser,runPar.getLatParams(), toolBox, runPar.dt, STRINGIFY(MODELLABEL)) //MODELLABEL is defined in the cmake.
    {
      ...
      //For convenience, we also define g as a function of lambda and q.
        ...
        alpha = 1;
        fStar = mu/sqrt(lambda); //fldS0[0];  //changed it, it should be mu
        omegaStar = sqrt(2)*mu;  // sqrt(lambda) * fStar;
   ...
   /////////
    ...
    {
      return -(mu/2) * pow<2>(fldS(0_c)) + (lambda/4) * pow<4>(fldS(0_c));  // (lambda/4)*pow((pow<2>(fldS(0_c)) - pow(eta,2)),2);
   ...
   /////////
    ...
    {
      return   -mu* fldS(0_c) + lambda * pow<3>(fldS(0_c)); // (lambda)*(pow<2>(fldS(0_c)) - pow(eta,2))*fldS(0_c);
   ...
   /////////
    ...
    {
      return   -mu + 3*lambda*pow<2>(fldS(0_c));  //(lambda)*(3* pow<2>(fldS(0_c)) - pow(eta,2)); // return  3 * pow<2>(fldS(0_c)) +  q * pow<2>(fldS(1_c)) ;
    }
