#!/bin/bash
# ─────────────────────────────────────────────────────────────────────────────
# run_z3_prs_stat_binfix.sh — rerun the N=512 Z3 *PRS* stat scan with the
#                             BINNING-FIXED DWZ3 binary, 3 seeds/config.
#
# Pair script: run_z3_phys_stat_binfix.sh (run that on the OTHER machine).
#
...
#   Per-run ~3.0-6.5 h.  Output -> NEW *_binfix dir (ORIGINALS PRESERVED).
#
# Usage:   ./run_z3_prs_stat_binfix.sh [MAX_PARALLEL_JOBS]
...
SCAN="scan_z3_prs_stat"                       # <<< this script: PRS only
...
wait
echo "════════ done: $total PRS runs. Output $OUT_ROOT/${SCAN}_binfix/, logs $LOG_DIR/ ════════"
